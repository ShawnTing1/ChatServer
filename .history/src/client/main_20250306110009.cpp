...
void readTaskHandler(int clientfd)
{
    for(;;) {
        ...
        if (ONE_CHAT_MSG == js["msgid"].get<int>()) {
            cout << js["time"].get<string>() << js["fromid"].get<int>() << " " << js["fromname"].get<string>() << " said: " << js["msg"].get<string>() << endl;
            continue;
        }
    }
}
...
void chat(int clientfd, string commandbuf)
{
    ...
    string message = commandbuf.substr(idx + 1, commandbuf.size() - idx - 1);
    ...
    cout << "send chat message: " << request << endl;
