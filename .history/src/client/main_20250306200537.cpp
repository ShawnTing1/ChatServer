                    ...
                    if (len == -1) {
                        ...
                        if (0 != response_js["errno"].get<int>()) {
                            ...
                            if (response_js.contains("offlinemsg")) {
                                json raw_data = response_js["offlinemsg"];
                            
                                // 如果 raw_data 是字符串，先解析为 JSON
                                if (raw_data.is_string()) {
                                    try {
                                        raw_data = json::parse(raw_data.get<string>());
                                    } catch (...) {
                                        cout << "Error: Failed to parse JSON string" << endl;
                                        return;
                                    }
                                }
                                ...
                                if (raw_data.is_array()) {
                                    ...
                                    cout << "Error: offlinemsg is not a valid JSON array" << endl;
                                }
                            }
